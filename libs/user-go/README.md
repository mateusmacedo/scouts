...
```go
// Go service consumindo user-node via HTTP
package main
...
func main() {
    ...
    // Usar user-go para processar
    for _, user := range users {
        result := gouser.GoUser(user["name"].(string))
        fmt.Println(result)
    }
}
```
...
```go
// Go service integrando com NestJS BFF
package main
...
func main() {
    ...
    result := gouser.GoUser(userReq.Name)
