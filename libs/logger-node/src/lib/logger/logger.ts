...
export class ComposedLogger implements Logger {
	...
	private async log(
		...
		try {
			...
			if (
				this.metrics &&
				typeof (this.metrics as LoggerWithMetrics)[level as keyof LoggerWithMetrics] === 'function'
			) {
				const metricsMethod = (this.metrics as LoggerWithMetrics)[
					level as keyof LoggerWithMetrics
				] as (message: string, fields?: Record<string, unknown>) => void | Promise<void>;
				if (metricsMethod) {
					metricsMethod(message, allFields);
				}
			}
