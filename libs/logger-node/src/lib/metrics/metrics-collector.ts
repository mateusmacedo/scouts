...
export class MetricsCollector extends BaseMetricsCollector {
	...
	private getRedactCount(): number {
		return this.metrics.redactCount || 0;
	}

	private getFlushCount(): number {
		return this.metrics.flushCount || 0;
	}

	private getBufferUtilization(): number {
		return this.metrics.bufferUtilization || 0;
	}

	private updateRedactLatency(): void {
		this.metrics.redactLatencyMs = this.calculateAverageLatency();
	}

	private updateBufferMetric(): void {
		this.metrics.bufferUtilization = Math.min(this.bufferSize / this.bufferCapacity, 1);
	}
