 ...
 * Demonstra integração entre bff-nest e notifier-express
 */
@Injectable()
export class NotificationsService {
	...
	async getNotificationStatus(notificationId: string): Promise<NotificationResponse | null> {
		...
		try {
			const response = await firstValueFrom(
				this.httpService
					.get<NotificationResponse>(`/api/v1/notifications/${notificationId}/status`, {
						headers: {
					...
					.pipe(
						retry({
							...
							delay: (_error, retryCount) => {
								const delay = this.retryDelay * 2 ** (retryCount - 1);
	...
	/**
	 * Enviar notificação por email via notifier-express
	 */
	private async sendEmailNotification(
		...
		const response = await firstValueFrom(
			this.httpService
				.post<NotificationResponse>('/api/v1/notifications/email', request, {
					headers: {
						'x-correlation-id': correlationId,
	...
	/**
	 * Enviar notificação por SMS via notifier-express
	 */
	private async sendSMSNotification(
		...
		const response = await firstValueFrom(
			this.httpService
				.post<NotificationResponse>('/api/v1/notifications/sms', request, {
					headers: {
						'x-correlation-id': correlationId,
						'Content-Type': 'application/json',
					},
